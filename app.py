...
if st.button("Predict Churn Risk (Using Real Model)"):
    try:
        ...
        X_input.loc[0, 'tenure_months'] = tenure_months
        X_input.loc[0, 'monthly_charges'] = monthly_charges
        X_input.loc[0, 'total_charges'] = total_charges
        X_input.loc[0, 'nps_score'] = nps_score
        X_input.loc[0, 'overage_charges'] = overage_charges
        X_input.loc[0, 'network_issues_3m'] = network_issues
        X_input.loc[0, 'avg_data_gb_month'] = avg_data_gb
        X_input.loc[0, 'service_rating_last_6m'] = service_rating
        ...
        X_input.loc[0, 'network_dissatisfaction'] = network_issues * 0.02
